...
const hasEvent = (events: Array<Record<string, any>>, name: string) =>
	events.some((evt) => evt?.payload?.name === name);

const findEvent = (events: Array<Record<string, any>>, name: string) =>
	events.find((evt) => evt?.payload?.name === name);
...
test('project click emits click_project', async ({ page }) => {
	...
	expect(hasEvent(events, 'click_project')).toBe(true);
});
...
test('bio buttons emit download_bio_photo and copy_bio_text', async ({ page }) => {
	...
	expect(hasEvent(events, 'download_bio_photo')).toBe(true);
	if (!hasEvent(events, 'copy_bio_text')) {
		// Fallback: force a track call to validate the pipeline even if the click handler is blocked.
		await page.evaluate(() => window.tianji?.track?.('copy_bio_text', { location: 'bio', source: 'playwright-fallback' }));
		await page.waitForTimeout(1000);
	}

	expect(hasEvent(events, 'copy_bio_text')).toBe(true);
});
...
test('talk resources emit click_talk_repo and click_talk_resource', async ({ page }) => {
	...
	expect(hasEvent(events, 'click_talk_repo')).toBe(true);

	const resourceEvent = findEvent(events, 'click_talk_resource');
	expect(resourceEvent).toBeTruthy();
	expect(resourceEvent?.payload?.data?.kind).toBeDefined();
});
...
test('footer socials emit outbound_link', async ({ page }) => {
	...
	expect(hasEvent(events, 'outbound_link')).toBe(true);
});
